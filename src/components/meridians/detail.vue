<template>
    ...
    >
        <div class="title">
            <h3>{{detail.name}}</h3>
            <span v-if="detail.maxLevel">{{ $t('第') }}{{detail.nowLevel}}/{{detail.maxLevel}}{{ $t('重') }}</span>
        </div>
        ...
        <template v-else>
            <div class="require">
                <p>{{ $t('需要等级：') }}{{detail.requireLevel}}</p>
                <span>消耗修为：{{detail.requirePower[detail.nowLevel]}}</span>
            </div>
            <div class="probability">
                <!-- <p>{{ $t('成功率：') }}{{detail.data[detail.nowLevel].probability}}</p>
            ...
            <div class="premise" v-if="detail.requireNode.length > 0">
                ...
                >{{item.name}}{{ $t('第') }}{{item.level}}{{ $t('重') }}</span>
            </div>
